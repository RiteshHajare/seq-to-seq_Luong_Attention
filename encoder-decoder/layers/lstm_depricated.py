...
class LSTM:
  def __init__(self,vocab_size,hidden_size,learning_rate,fromEncoder,output_dims):
    ...
    # Final Gate
    self.wfi = initWeights((2 if self.fromEncoder else 1)*hidden_size,output_dims)
  ...
  def reset(self,lstm):
    ...
    self.encoder_outputs = lstm.encoder_outputs if lstm else []
    self.context = {}  # Store context vectors for attention
  ...
  def forward(self,input,idx):
    ...
    if self.fromEncoder:
      decoder_hidden = self.short_mem[idx]  # shape: [1, hidden_size]
      encoder_outputs = np.vstack(self.encoder_outputs) if isinstance(self.encoder_outputs, list) and len(self.encoder_outputs) > 0 else self.encoder_outputs
      context, attn_weights = self.compute_attention(decoder_hidden, encoder_outputs)
      self.context[idx] = context  # Store context for use in backward
      combined = np.concatenate((decoder_hidden, context), axis=1)  # shape: [1, hidden_size*2]
    ...
    # Final Output
      ...
      self.encoder_outputs.append(self.short_mem[idx].squeeze())
